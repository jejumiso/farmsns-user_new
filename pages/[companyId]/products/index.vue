      ...
      >
        ...
        <div class="p-2 w-full text-center">
          ...
          <div class="text-xs font-bold mt-1 text-center">
            <template v-if="isFriendtalkSpecial(product)">
            ...
            <template v-else>
              <div class="text-green-600 font-bold">
                {{ getEffectivePrice(product).toLocaleString() + '원' }}
              </div>
            </template>
          </div>
        </div>
...
const openMessage = computed(() => {
  if (isOpen.value) return '✅ 영업중입니다. 많은 이용 부탁드립니다!'
...
const isFriendtalkSpecial = (product: any) => {
  return (
...
const isFakeMode = computed(() =>
  authStore.customerProfile?.roles?.includes('fake') ?? false
)
</script>
